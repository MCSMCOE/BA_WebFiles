                    ...
                    <div class="bnr-inr bg-pt" style="background-image:url(/images/banner/department/<?php echo $id ?>.jpg);">
                        ...
                        <div class="container">
                            ...
                            <div class="bnr-inr-entry text-center">
                                <h1 class="text-capitalize"><span>Consultancy Projects</span></h1>
                                <!-- Breadcrumb row -->
                                ...
                                <div class="col-lg-12 col-md-10 m-t15 col-sm-6 m-b30">
                                    <table id="mydTable" class="ui celled table display table table-striped table-bordered"
                                        ...
                                        <tbody>
                                            ...
                                            $qry_category= "SELECT rm.rm_id,CONCAT(IFNULL(spv.staff_id,''))ss_no,GROUP_CONCAT(DISTINCT spv.legend,' ',spv.staff_name) staff_name ,GROUP_CONCAT(IF(av.am_id='196',alv.list_value,'') SEPARATOR '') item0,GROUP_CONCAT(IF(av.am_id='60',av.txt_val,'') SEPARATOR '') industry_name,GROUP_CONCAT(IF(av.am_id='61',av.txt_val,'') SEPARATOR '') industry_address,GROUP_CONCAT(DISTINCT IF(av.am_id='35',av.txt_val,'') SEPARATOR '') amount,GROUP_CONCAT(IF(av.am_id='37',DATE_FORMAT(av.date_val,'%d-%m-%Y'),'') SEPARATOR '') sans_date,GROUP_CONCAT(IF(av.am_id='38',DATE_FORMAT(av.date_val,'%d-%m-%Y'),'') SEPARATOR '') start_date,GROUP_CONCAT(IF(av.am_id='39',DATE_FORMAT(av.date_val,'%d-%m-%Y'),'') SEPARATOR '') end_date,GROUP_CONCAT(IF(av.am_id='9',av.txt_val,'') SEPARATOR '') item7,GROUP_CONCAT(IF(av.am_id='10',av.txt_val,'') SEPARATOR '') item8,GROUP_CONCAT(IF(av.am_id='148',av.int_val,'') SEPARATOR '') item9,GROUP_CONCAT(IF(av.am_id='149',av.int_val,'') SEPARATOR '') item10,GROUP_CONCAT(IF(av.am_id='159',av.txt_val,'') SEPARATOR '') item11,GROUP_CONCAT(IF(av.am_id='194',av.int_val,'') SEPARATOR '') item12,GROUP_CONCAT(am.list_qy SEPARATOR '') qy12 FROM documentation.record_master rm INNER JOIN documentation.record_ss_mapping rsm  ON rm.rm_id=rsm.rm_id  AND rm.status>0 AND rsm.status>0 AND rm.im_id='38' INNER JOIN camps.staff_personal_copy spv ON spv.staff_id = rsm.ss_id AND spv.dep_id='".mysqli_real_escape_string($dbcon, $id)."' AND LENGTH(spv.staff_id )=LENGTH(rsm.ss_id)  LEFT JOIN (documentation.attr_values av INNER JOIN documentation.attribute_master am ON am.am_id=av.am_id AND am.status>0 AND av.status>0 ) ON av.rm_id=rm.rm_id  AND av.status>0   LEFT JOIN documentation.attribute_list_values alv ON alv.am_id=av.am_id AND alv.status>0 AND alv.alv_id=av.int_val GROUP BY rm.rm_id HAVING TRUE";
                                            ...
                                            <tr>
                                                ...
                                                <td><?php echo $rows_mastercategory['industry_name']; ?></td>
                                                <td><?php echo $rows_mastercategory['industry_address']; ?></td>
                                                 <td><?php echo $rows_mastercategory['amount']; ?></td>
